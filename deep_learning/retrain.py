...
tf.app.flags.DEFINE_string('image_dir', '/Volumes/SEAGATE/zalando_src_images/total_images/',
                           """Path to folders of labeled images.""")
tf.app.flags.DEFINE_string('output_graph', '/Volumes/SEAGATE/zalando_src_images/output_graph.pb',
                           """Where to save the trained graph.""")
tf.app.flags.DEFINE_string('output_labels', '/Volumes/SEAGATE/zalando_src_images/output_labels.txt',
                           """Where to save the trained graph's labels.""")
tf.app.flags.DEFINE_string('summaries_dir', '/Volumes/SEAGATE/zalando_src_images/retrain_logs',
...
tf.app.flags.DEFINE_string('model_dir', '/Users/mr.narendhrancs/Desktop/imagenet',
                           """Path to classify_image_graph_def.pb, """
                           """imagenet_synset_to_human_label_map.txt, and """
                           """imagenet_2012_challenge_label_map_proto.pbtxt.""")
tf.app.flags.DEFINE_string(
    'bottleneck_dir', '/Volumes/SEAGATE/zalando_src_images/bottleneck',
...
MODEL_INPUT_WIDTH = 762 #299
MODEL_INPUT_HEIGHT = 1100
MODEL_INPUT_DEPTH = 3
...
def get_or_create_bottleneck_with_threading(necessary_datas):
  while True:
      try:


            necessary_data=necessary_datas.get()
            sess = necessary_data['sess']
            image_lists = necessary_data['image_lists']
            label_name = necessary_data['label_name']
            index = necessary_data['index']
            image_dir = necessary_data['image_dir']
            category = necessary_data['category']
            bottleneck_dir = necessary_data['bottleneck_dir']
            jpeg_data_tensor = necessary_data['jpeg_data_tensor']
            bottleneck_tensor = necessary_data['bottleneck_tensor']
...
def cache_bottlenecks_with_threading(sess, image_lists, image_dir, bottleneck_dir,
                      jpeg_data_tensor, bottleneck_tensor):
    """Ensures all the training, testing, and validation bottlenecks are cached.
  ...
  """
    ...
    for thread in range(no_of_threads):
        worker = Thread(target=get_or_create_bottleneck_with_threading, args=(myQueue,))
...
def main(_):
    ...
    if do_distort_images:
        ...
        if NUM_THREADS is None:
            ...
            cache_bottlenecks_with_threading(sess, image_lists, FLAGS.image_dir, FLAGS.bottleneck_dir,
                              jpeg_data_tensor, bottleneck_tensor)
    ...
    for i in range(FLAGS.how_many_training_steps):
        ...
        if (i % 1000) == 0 :
            tensorname = FLAGS.final_tensor_name
            output_graph = FLAGS.output_graph + str(i)
            output_labels = FLAGS.output_labels + str(i)
            # Write out the TEMPORARY trained graph and labels with the weights stored as constants.
            output_graph_def = graph_util.convert_variables_to_constants(
                sess, graph.as_graph_def(), [tensorname])
